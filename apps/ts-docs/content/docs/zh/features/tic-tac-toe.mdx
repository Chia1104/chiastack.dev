---
title: TicTacToe
description: 井字遊戲狀態管理
---

`@chiastack/features/tic-tac-toe` 提供了一個完整的井字遊戲（Tic-Tac-Toe）狀態管理解決方案，支援可自訂大小的遊戲板。

## 安裝

<Tabs items={['npm', 'pnpm', 'yarn', 'bun']}>
  <Tab value="npm">
    ```bash
    npm install @chiastack/features
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @chiastack/features
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @chiastack/features
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @chiastack/features
    ```
  </Tab>
</Tabs>

## 匯入

```typescript
import {
  TicTacToeStoreProvider,
  useTicTacToeStore,
} from '@chiastack/features/tic-tac-toe/store';
```

## 基本使用

### 1. 設定 Provider

首先，在應用程式的根元件或需要使用的區域包裹 `TicTacToeStoreProvider`：

```tsx
import { TicTacToeStoreProvider } from '@chiastack/features/tic-tac-toe/store';

function App() {
  return (
    <TicTacToeStoreProvider>
      <GameBoard />
    </TicTacToeStoreProvider>
  );
}
```

### 2. 使用 Store

在元件中使用 `useTicTacToeStore` Hook 來存取和操作遊戲狀態：

```tsx
import { useTicTacToeStore } from '@chiastack/features/tic-tac-toe/store';

function GameBoard() {
  const board = useTicTacToeStore((state) => state.board);
  const currentPlayer = useTicTacToeStore((state) => state.currentPlayer);
  const winner = useTicTacToeStore((state) => state.winner);
  const isDraw = useTicTacToeStore((state) => state.isDraw);
  const makeMove = useTicTacToeStore((state) => state.makeMove);
  const resetGame = useTicTacToeStore((state) => state.resetGame);

  return (
    <div>
      <div>當前玩家: {currentPlayer}</div>
      {winner && <div>獲勝者: {winner}</div>}
      {isDraw && <div>平手！</div>}
      
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.sqrt(board.length)}, 1fr)` }}>
        {board.map((cell, index) => (
          <button
            key={index}
            onClick={() => makeMove(index)}
            disabled={cell !== null || winner !== null || isDraw}
          >
            {cell || '-'}
          </button>
        ))}
      </div>
      
      <button onClick={resetGame}>重新開始</button>
    </div>
  );
}
```

## API 參考

### TicTacToeStoreProvider

提供 TicTacToe Store 的 Context Provider。

#### Props

```typescript
interface TicTacToeStoreProviderProps {
  children: React.ReactNode;
  initialState?: Partial<InitialState>;
}
```

- `children`: React 子元件
- `initialState`: 可選的初始狀態，用於覆蓋預設值

#### 範例

```tsx
<TicTacToeStoreProvider initialState={{ size: 4 }}>
  <YourComponent />
</TicTacToeStoreProvider>
```

### useTicTacToeStore

用於存取和操作 TicTacToe Store 的 Hook。

#### 語法

```typescript
const value = useTicTacToeStore<T>(selector, name?);
```

- `selector`: 選擇器函式，用於從 store 中選取需要的資料
- `name`: 可選的錯誤訊息名稱（預設為 "useTicTacToeStore"）

### State

```typescript
interface InitialState {
  size: number;              // 遊戲板大小（預設為 3，表示 3x3）
  board: Board;              // 遊戲板狀態
  currentPlayer: Player;     // 當前玩家（'X' 或 'O'）
  winner: Player | null;     // 獲勝者（'X'、'O' 或 null）
  isFull: boolean;          // 遊戲板是否已滿
  isDraw: boolean;          // 是否平手
  winPatterns: number[][];   // 獲勝模式（內部使用）
}

type Player = 'X' | 'O' | null;
type Board = Player[];
```

### Actions

#### makeMove

在指定位置下棋。

```typescript
makeMove: (index: number) => void;
```

**參數：**
- `index`: 棋盤位置索引（0 到 size² - 1）

**行為：**
- 如果該位置已有棋子、遊戲已結束或平手，則不會執行任何操作
- 下棋後會自動切換玩家
- 會自動檢查是否有獲勝者或平手

**範例：**

```typescript
makeMove(0);  // 在左上角下棋
```

#### resetGame

重置遊戲到初始狀態。

```typescript
resetGame: () => void;
```

**行為：**
- 清空棋盤
- 重置當前玩家為 'X'
- 清除獲勝者和平手狀態
- 保持當前的遊戲板大小

**範例：**

```typescript
resetGame();
```

#### changeSize

改變遊戲板大小並重置遊戲。

```typescript
changeSize: (newSize: number) => void;
```

**參數：**
- `newSize`: 新的遊戲板大小（例如 3 表示 3x3，4 表示 4x4）

**行為：**
- 改變遊戲板大小
- 重置遊戲到初始狀態
- 重新計算獲勝模式

**範例：**

```typescript
changeSize(4);  // 改為 4x4 遊戲板
```

## 完整範例

```tsx
import { TicTacToeStoreProvider, useTicTacToeStore } from '@chiastack/features/tic-tac-toe/store';

function App() {
  return (
    <TicTacToeStoreProvider>
      <TicTacToeGame />
    </TicTacToeStoreProvider>
  );
}

function TicTacToeGame() {
  const board = useTicTacToeStore((state) => state.board);
  const currentPlayer = useTicTacToeStore((state) => state.currentPlayer);
  const winner = useTicTacToeStore((state) => state.winner);
  const isDraw = useTicTacToeStore((state) => state.isDraw);
  const size = useTicTacToeStore((state) => state.size);
  const makeMove = useTicTacToeStore((state) => state.makeMove);
  const resetGame = useTicTacToeStore((state) => state.resetGame);
  const changeSize = useTicTacToeStore((state) => state.changeSize);

  const gameOver = winner !== null || isDraw;
  const canMakeMove = !gameOver;

  return (
    <div>
      <h1>井字遊戲</h1>
      
      <div>
        <label>
          遊戲板大小:
          <select
            value={size}
            onChange={(e) => changeSize(Number(e.target.value))}
            disabled={!gameOver}
          >
            <option value={3}>3x3</option>
            <option value={4}>4x4</option>
            <option value={5}>5x5</option>
          </select>
        </label>
      </div>

      <div>
        {gameOver ? (
          winner ? (
            <div>🎉 玩家 {winner} 獲勝！</div>
          ) : (
            <div>🤝 平手！</div>
          )
        ) : (
          <div>當前玩家: {currentPlayer}</div>
        )}
      </div>

      <div
        style={{
          display: 'grid',
          gridTemplateColumns: `repeat(${size}, 80px)`,
          gap: '4px',
          margin: '20px 0',
        }}
      >
        {board.map((cell, index) => (
          <button
            key={index}
            onClick={() => makeMove(index)}
            disabled={!canMakeMove || cell !== null}
            style={{
              width: '80px',
              height: '80px',
              fontSize: '24px',
              cursor: canMakeMove && cell === null ? 'pointer' : 'not-allowed',
            }}
          >
            {cell || ''}
          </button>
        ))}
      </div>

      {gameOver && (
        <button onClick={resetGame}>重新開始</button>
      )}
    </div>
  );
}
```

## 遊戲規則

- 遊戲由兩個玩家輪流下棋，玩家 'X' 先手
- 玩家輪流在空位上放置自己的標記（X 或 O）
- 當有玩家在橫、直或斜線上連成一線時獲勝
- 當棋盤填滿且無人獲勝時為平手
- 支援自訂大小的遊戲板（3x3、4x4、5x5 等）

## 注意事項

- `useTicTacToeStore` 必須在 `TicTacToeStoreProvider` 內部使用，否則會拋出錯誤
- Store 使用 Zustand 的 DevTools 整合，可以在開發時使用 Redux DevTools 進行除錯
- 遊戲板大小改變時會自動重置遊戲
- 獲勝模式會根據遊戲板大小自動計算

